...
Reuses the shared ``upscaler`` AI component (torch_runtime + realesrgan), so the
install banner here is identical to the image upscaler's — installing one
installs both.
"""
...
class VideoUpscalerSection(QScrollArea):
    """Real-ESRGAN video upscaler (x2 / x4) — resumable frame pipeline."""
    ...
    def __init__(self, settings, parent=None) -> None:
        ...
        # Shares the image upscaler's component — torch + realesrgan.
    ...
    def _build_source_card(self) -> QFrame:
        ...
        self._hint_src = QLabel(tr("hint_video_upscaler_source"))
        ...
        self._input_edit.setPlaceholderText(tr("ph_video"))
        ...
        self._probe_lbl = QLabel("")
        self._probe_lbl.setObjectName("TextMuted")
        self._probe_lbl.setStyleSheet("font-size: 11px; color: #3B82F6;")
        self._probe_lbl.setVisible(False)
        layout.addWidget(self._probe_lbl)
        return card
    ...
    def _build_output_card(self) -> QFrame:
        ...
        self._output_edit.setPlaceholderText(tr("ph_video_upscaler_output"))
    ...
    def _build_options_card(self) -> QFrame:
        ...
        self._scale_rb_2.setChecked(True)  # 2× recommended for video
        scale_row.addStretch()
        layout.addLayout(scale_row)
        self._scale_hint = QLabel(tr("hint_video_upscaler_scale"))
        self._scale_hint.setObjectName("TextMuted")
        self._scale_hint.setWordWrap(True)
        self._scale_hint.setStyleSheet("font-size: 11px;")
        layout.addWidget(self._scale_hint)

        self._hdr_model = _section_header(tr("hdr_video_upscaler_model"))
        layout.addWidget(self._hdr_model)
        self._model_combo = QComboBox()
        self._model_combo.setObjectName("PillInput")
        self._model_combo.addItem(tr("video_upscaler_model_fast"), "general")
        self._model_combo.addItem(tr("video_upscaler_model_quality"), "x4plus")
        self._model_combo.setCurrentIndex(0)  # fast/light default
        layout.addWidget(self._model_combo)
        self._model_hint = QLabel(tr("hint_video_upscaler_model"))
    ...
    def _build_device_card(self) -> QFrame:
        ...
        self._warn_lbl = QLabel(f"⚠ {tr('warn_video_upscaler_time')}")
    ...
    def _on_device_detected(self, payload) -> None:
        ...
        # CPU video upscaling is impractically slow — make that loud.
        self._warn_lbl.setText(
            f"⚠ {tr('warn_video_upscaler_cpu')}" if device == "cpu"
            else f"⚠ {tr('warn_video_upscaler_time')}"
        )
    ...
    def retranslate_ui(self) -> None:
        ...
        self._hint_src.setText(tr("hint_video_upscaler_source"))
        self._input_edit.setPlaceholderText(tr("ph_video"))
        ...
        self._scale_hint.setText(tr("hint_video_upscaler_scale"))
        self._hdr_model.setText(tr("hdr_video_upscaler_model"))
        self._model_combo.setItemText(0, tr("video_upscaler_model_fast"))
        self._model_combo.setItemText(1, tr("video_upscaler_model_quality"))
        self._model_hint.setText(tr("hint_video_upscaler_model"))
        self._hdr_result.setText(tr("hdr_result"))
        self._open_btn.setText(tr("btn_open_explorer"))
        dev = getattr(self, "_detected_device", None)
        self._warn_lbl.setText(
            f"⚠ {tr('warn_video_upscaler_cpu')}" if dev == "cpu"
            else f"⚠ {tr('warn_video_upscaler_time')}"
        )
    ...
    def _selected_model(self) -> str:
        return self._model_combo.currentData() or "general"

    def _browse_input(self) -> None:
        from core.video_upscaler import VIDEO_EXTS
    ...
    def _on_input_changed(self, text: str) -> None:
        """Probe the chosen video and show a realistic frame-count / time hint."""
        path = text.strip()
        ...
        def _do_probe():
            try:
                from core.video_upscaler import probe_video
                return probe_video(path)
            except Exception:
                return None

        w = Worker(_do_probe)
        w.signals.result.connect(self._on_probe_result)
        w.start()
        self._probe_worker = w

    def _on_probe_result(self, info) -> None:
        ...
        self._probe_lbl.setText(
            tr("video_upscaler_probe_info").format(
                width=info["width"], height=info["height"],
                frames=info["frame_count"], mins=mins, secs=secs,
            )
        )
        self._probe_lbl.setVisible(True)
    ...
    def _on_progress(self, value: int, _total: int, label: str) -> None:
        ...
        self._progress_lbl.setText(label or f"{value}%")
    ...
    def trigger_primary_action(self) -> None:
        if self._worker and self._worker.isRunning():
            self._worker.cancel()
            self.status_message.emit(tr("video_upscaler_cancelling"), False)
            return

        from core.video_upscaler import VIDEO_EXTS
        input_path = self._input_edit.text().strip()
        if not input_path or not os.path.isfile(input_path):
            self.status_message.emit(tr("err_video_upscaler_no_input"), True)
            return
        if os.path.splitext(input_path)[1].lower() not in VIDEO_EXTS:
            self.status_message.emit(tr("err_video_upscaler_not_video"), True)
            return
        ...
        self._set_busy(True, tr("video_upscaler_processing"))

        def _progress_cb(pct: int, msg: str) -> None:
            if self._worker:
                self._worker.signals.progress.emit(pct, 100, msg)

        def _cancelled_cb() -> bool:
            return bool(self._worker and self._worker.is_cancelled)

        from core.video_upscaler import upscale_video
        if self._device_rb_cpu.isChecked():
            device_override: str | None = "cpu"
    ...
    def _on_result(self, result: dict) -> None:
        ...
        if not result.get("success"):
            ...
            if result.get("resumable"):
                err += "  " + tr("video_upscaler_resume_hint")
            self.status_message.emit(f"{tr('video_upscaler_failed')}: {err}", True)
            return

        self._last_output_path = result["output_path"]
        self._result_lbl.setText(
            f"🎬 {tr('lbl_upscaler_result')}: {os.path.basename(self._last_output_path)} "
            f"({result['scale']}× · {result['frame_count']} frames)"
        )
        ...
        self.status_message.emit(
            f"{tr('video_upscaler_done')} [{result['device'].upper()}] → "
            f"{os.path.basename(self._last_output_path)}", False
        )
