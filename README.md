...
- Document Convert reads PDF, DOCX and **PPTX**, and targets PDF, DOCX and **MD** (Markdown). Markdown is an output format only — converting *from* markdown is deliberately not offered; instead the section has a **Markdown Viewer** sub-tab: a VSCode-style source pane on the left (syntax highlighting, line-number gutter, current-line highlight, wrap toggle, live line/word/char count) and a live preview on the right, rendered by Qt's native `setMarkdown()` so there is no extra dependency. The two panes are **scroll-synced** proportionally in both directions (toggleable), with open/save/save-as and a 180 ms debounce so typing does not re-parse every keystroke. Preview is skipped above 2 MB, where a synchronous re-render would freeze the window on every keystroke. PPTX is text-only by design (slide titles become headings, body text becomes nested bullets, tables are kept, pictures/charts/SmartArt are skipped) and converts to MD or DOCX, not PDF. PDF → MD reuses the same layout analysis as PDF → DOCX, so heading levels and lists are inferred and may need a tidy-up; DOCX → MD reads Word's own heading/list/table structure and is exact. Images are skipped in Markdown output (it can't embed them) and reported in the summary; a scanned PDF page yields a warning rather than text
