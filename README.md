...
- **Local Adjustments (Masks)** — pro-style selective editing: stack multiple masks of five types — **radial** (feathered ellipse, now **rotatable**), **linear gradient**, **color-range** (eyedropper — click the sky, grass or any colour, tolerance slider selects every matching pixel), **luminance-range** (select by brightness — target just the shadows or just the highlights), and **brush** (paint the selection freehand on the preview, with an **erase** mode). Each mask applies a full adjustment set — brightness, contrast, saturation, hue, temperature, tint, exposure, shadows, highlights, **clarity, dehaze, vibrance, gamma, sharpen and blur** — to just its region. **Combine** modes let masks build on each other: *Add* starts a new selection, *Subtract* and *Intersect* refine the mask above it. Per-mask **opacity**, **feather** and **invert**; **reorder** and **duplicate** masks; **Show mask overlay** tints every selected region red so you see exactly what is affected; masks composite in order and stay non-destructive
